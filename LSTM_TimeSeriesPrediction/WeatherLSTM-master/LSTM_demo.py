...
if model_type == 3:
    # Bidirectional LSTM
    model = Sequential()
    model.add(Bidirectional(LSTM(200, activation='relu'), input_shape=(X_train.shape[1], 1)))
    model.add(Dense(1))
...
history = model.fit(X_train, y_train, epochs = 150, batch_size = 6)
...
print("Accuracy: %.2f%%" % (y_predicted[1]*100))
#
